...
# Check if Radarr is already installed and running
if systemctl is-active --quiet radarr 2>/dev/null; then
    echo -e "${GREEN}✅ Radarr is already installed and running${NC}"
    echo -e "${BLUE}ℹ️  Access via http://$(hostname -I | awk '{print $1}'):7878${NC}"
    echo -e "${YELLOW}⚠️  Skipping installation (service already active)${NC}"
    exit 0
fi

echo -e "${CYAN}🎬 Installing Radarr (WAWYC Method)...${NC}"

# Radarr installation using automated approach (WAWYC method)
echo -e "${YELLOW}  → Installing Radarr automatically${NC}"

# Set variables for Radarr
app="radarr"
app_port="7878"
...
echo -e "${YELLOW}  → Creating radarr user and setting up groups${NC}"
...
echo -e "${YELLOW}  → Downloading Radarr${NC}"
...
if ! wget --content-disposition "$DLURL"; then
    echo -e "${RED}❌ Failed to download Radarr${NC}"
    exit 1
fi

echo -e "${YELLOW}  → Extracting and installing Radarr${NC}"
tar -xzf "${app^}".*.tar.gz
rm -rf "$bindir"
mv "${app^}" $installdir
...
echo -e "${GREEN}✓ Radarr installation completed${NC}"

# Create systemd service
echo -e "${YELLOW}  → Creating Radarr systemd service${NC}"
...
echo -e "${YELLOW}  → Starting Radarr service${NC}"
...
if systemctl is-active --quiet radarr; then
    echo -e "${GREEN}✓ Radarr service started successfully${NC}"
else
    echo -e "${RED}❌ Radarr service failed to start${NC}"
    systemctl status radarr --no-pager
    exit 1
fi

# Get server IP (VPN-aware method)
get_server_ip() {
    local ip=""
...
echo ""
echo -e "${GREEN}✅ Radarr installation completed successfully!${NC}"
echo ""
echo -e "${CYAN}🌐 Web Interface Access:${NC}"
echo -e "${WHITE}   URL: ${BLUE}http://$SERVER_IP:7878${NC}"
echo ""
echo -e "${BLUE}📖 Configuration Recommendations:${NC}"
echo -e "${WHITE}   • Media folder: ${CYAN}/srv/serverFilesystem/media/movies${NC}"
